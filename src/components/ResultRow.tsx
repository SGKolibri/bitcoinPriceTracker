...
export default function ResultRow({
  ...
  const url = `http://${providerName}.com`;
  
  return (
    <a
      href={url}
      target="_blank"
      className="block relative border min-h-[64px] border-white/10 rounded-lg bg-gradient-to-r from-purple-500/10 to-sky-300/10 p-4 my-2 overflow-hidden">
        ...
        {btc && (
          <div className="flex gap-2">
            <span className="text-xl text-purple-200/80">
              { new Intl.NumberFormat('pt-BR',  {minimumFractionDigits:8}).format(parseFloat(btc))}
            </span>
